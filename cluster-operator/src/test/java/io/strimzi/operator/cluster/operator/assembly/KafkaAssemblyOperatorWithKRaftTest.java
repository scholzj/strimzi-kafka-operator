...
public class KafkaAssemblyOperatorWithKRaftTest {
    ...
    /**
     * Helper method to create an empty Secret with metadata (in Kubernetes, metadata is never null)
     */
    private static Secret mockOperatorSecret() {
    ...
    public void testRegularReconciliation(VertxTestContext context)  {
        ...
        when(secretOps.getAsync(any(), any())).thenReturn(Future.succeededFuture());
    ...
    public void testScaleUp(VertxTestContext context)  {
        ...
        SecretOperator secretOps = supplier.secretOperations;
        when(secretOps.reconcile(any(), any(), any(), any())).thenReturn(Future.succeededFuture());
        when(secretOps.getAsync(any(), any())).thenReturn(Future.succeededFuture(mockOperatorSecret()));
        when(secretOps.listAsync(any(), any(Labels.class))).thenReturn(Future.succeededFuture(List.of()));
        ...
        kao.reconcile(new Reconciliation("test-trigger", Kafka.RESOURCE_KIND, NAMESPACE, CLUSTER_NAME))
                .onComplete(context.succeeding(v -> context.verify(() -> {
                    ...
                    verify(mockAdmin, times(1)).describeCluster(any(DescribeClusterOptions.class));
    ...
    public void testScaleDown(VertxTestContext context)  {
        ...
        when(dcr.nodes()).thenReturn(KafkaFuture.completedFuture(
                List.of(
                        new Node(3, "my-cluster-broker-3", 9092),
                        new Node(4, "my-cluster-broker-4", 9092),
                        new Node(5, "my-cluster-broker-5", 9092),
                        new Node(6, "my-cluster-broker-6", 9092),
                        new Node(7, "my-cluster-broker-7", 9092)
                        )
                )
        );
        when(mockAdmin.describeCluster(any(DescribeClusterOptions.class))).thenReturn(dcr);
    ...
    public void testUnregistrationFailureWithScaleUp(VertxTestContext context)  {
        ...
        when(dcr.nodes()).thenReturn(KafkaFuture.completedFuture(
                        List.of(
                                new Node(3, "my-cluster-broker-3", 9092),
                                new Node(4, "my-cluster-broker-4", 9092),
                                new Node(1874, "my-cluster-broker-1874", 9092),
                                new Node(1919, "my-cluster-broker-1919", 9092)
                        )
                )
        );
        when(mockAdmin.describeCluster(any(DescribeClusterOptions.class))).thenReturn(dcr);
    ...
    public void testNoNodePoolsValidation(VertxTestContext context)  {
        ...
        when(secretOps.reconcile(any(), any(), any(), any())).thenReturn(Future.succeededFuture());
        when(secretOps.listAsync(any(), any(Labels.class))).thenReturn(Future.succeededFuture(List.of()));
        when(secretOps.getAsync(eq(NAMESPACE), eq(KafkaResources.clusterOperatorCertsSecretName(CLUSTER_NAME)))).thenReturn(Future.succeededFuture(mockOperatorSecret()));
